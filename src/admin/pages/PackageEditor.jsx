...
const EMPTY = {
  title: '', slug: '', duration: '', price: 0, oldPrice: null, priceSuffix: '/ package', capacity: '',
  active: true, featured: false, shortDescription: '', description: '', image: '', gallery: [],
  inclusions: [], exclusions: [], itinerary: [], highlights: [], amenities: [], faqs: [],
...
export default function PackageEditor() {
  ...
  const { item, setItem, isNew, loading, saving, error, dirty, save, discard } = useItemEditor('packages', EMPTY, '/admin/packages');

  useUnsavedChangesWarning(dirty);

  if (loading || !item) return <LoadingSpinner label="Loading package…" />;
  ...
  const handleDelete = async () => {
    ...
    try {
      await contentApi.remove('packages', item.id);
  ...
  return (
    <EditorShell
      title={isNew ? 'New Package' : item.title}
      ...
      previewHref={isNew ? null : `/packages/${item.slug}`}
    >
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="space-y-6 rounded-2xl border border-slate-200 bg-white p-5 lg:col-span-2">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <ListEditor label="Itinerary" items={item.itinerary} onChange={(v) => set({ itinerary: v })} />
